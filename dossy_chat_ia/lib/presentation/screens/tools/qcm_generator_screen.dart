...
class QcmGeneratorScreen extends StatefulWidget {
  const QcmGeneratorScreen({super.key});
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                isFr ? 'Génération du QCM...' : 'Generating QCM...',
...
${content.isEmpty ? '' : 'Contenu du cours:\n"""\n$content\n"""'}''';
      ...
      if (resp['success'] != true) {
        throw Exception(resp['message'] ?? 'Échec génération QCM');
      }
      ...
      final List qs = data['questions'] ?? [];
      final parsed = qs.map<Map<String, dynamic>>((q) {
      ...
      print('🔔 Calling incrementStat for quizzes_created');
      authProvider.incrementStat('quizzes_created').catchError((e) {
        print('❌ Error incrementing quizzes: $e');
      });
    } catch (e) {
      Navigator.pop(context);
      if (mounted) {
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [AppConstants.primaryGreen, AppConstants.primaryGreen.withAlpha((0.7 * 255).round())],
            ...
            child: Column(
              children: [
                ...
                Text(
                  '$percentage%',
                  style: TextStyle(fontSize: 20, color: Colors.white.withAlpha((0.9 * 255).round())),
                ),
              ],
            ),
          ),
