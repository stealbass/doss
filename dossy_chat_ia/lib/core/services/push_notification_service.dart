...
class PushNotificationService {
  ...
  Future<void> initialize() async {
    try {
      ...
      if (settings.authorizationStatus == AuthorizationStatus.authorized) {
        print('✅ Push notifications: Permissions accordées');
      } else if (settings.authorizationStatus == AuthorizationStatus.provisional) {
        print('⚠️ Push notifications: Permissions provisoires');
      } else {
        print('❌ Push notifications: Permissions refusées');
        return;
      }
      ...
      print('✅ PushNotificationService initialisé avec succès');
    } catch (e) {
      print('❌ Erreur initialisation PushNotificationService: $e');
    }
  }
  ...
  void _setupMessageHandlers() {
    ...
    FirebaseMessaging.instance.getInitialMessage().then((RemoteMessage? message) {
      if (message != null) {
        print('🚀 Notification cliquée (app fermée): ${message.notification?.title}');
        _messageStreamController.add(message);
        _handleNotificationData(message.data);
      }
    });
  ...
  void _handleNotificationData(Map<String, dynamic> data) {
    ...
    switch (type) {
      ...
      case 'audience':
        ...
        print('🏛️ Naviguer vers audience: $id');
      ...
      case 'legal_alert':
        ...
        print('⚖️ Naviguer vers alertes juridiques');
        // TODO: Implémenter navigation
      ...
      case 'template':
        ...
        print('📄 Naviguer vers documents');
        // TODO: Implémenter navigation
        // Navigator.pushNamed(context, '/documents');
        break;

      default:
        print('⚠️ Type de notification inconnu: $type');
    }
  }
  ...
  Future<void> _sendTokenToBackend(String token) async {
    try {
      ...
      if (apiToken == null || baseUrl == null) {
        print('⚠️ Pas de token API ou URL backend configurés');
        return;
      }
      ...
      if (response.statusCode == 200) {
        ...
        print('❌ Erreur envoi FCM token au backend: ${response.statusCode}');
      }
    } catch (e) {
      print('❌ Erreur envoi FCM token au backend: $e');
    }
  }
