...
class OfflineService {
  ...
  Future<void> initialize() async {
    try {
      ...
      _log('🌐 Statut connexion initial: ${_isOnline ? "ONLINE" : "OFFLINE"}');
      ...
      _connectivitySubscription = _connectivity.onConnectivityChanged
        ...
        _log('🌐 Changement connexion: ${_isOnline ? "ONLINE" : "OFFLINE"}');
        _connectionStatusController.add(_isOnline);

        // Si on vient de se reconnecter, synchroniser
        if (!wasOnline && _isOnline) {
          _log('🔄 Connexion rétablie, synchronisation...');
          _syncPendingActions();
        }
      });

      _log('✅ OfflineService initialisé avec succès');
    } catch (e) {
      _log('❌ Erreur initialisation OfflineService: $e');
    }
  }
  ...
  Future<void> cacheData(String key, dynamic data) async {
    try {
      ...
      _log('💾 Données cachées: $key');
    } catch (e) {
      _log('❌ Erreur cache données: $e');
    }
  }
    ...
    try {
      ...
      if (maxAge != null) {
        ...
        if (timestampStr != null) {
          ...
          if (age > maxAge) {
            _log('⏰ Cache expiré pour: $key (âge: ${age.inMinutes} min)');
            return null;
          }
        }
      }
      ...
      _log('📦 Données récupérées du cache: $key');
      return data;
    } catch (e) {
      _log('❌ Erreur récupération cache: $e');
      return null;
    }
  }
  ...
  Future<void> deleteCachedData(String key) async {
    try {
      ...
      _log('🗑️ Cache supprimé: $key');
    } catch (e) {
      _log('❌ Erreur suppression cache: $e');
    }
  }
  ...
  Future<void> clearCache() async {
    try {
      ...
      _log('🗑️ Tout le cache a été vidé');
    } catch (e) {
      _log('❌ Erreur vidage cache: $e');
    }
  }
  ...
  Future<void> addPendingAction({
    ...
    try {
      ...
      _log('📝 Action ajoutée à la file d\'attente: $type');
    } catch (e) {
      _log('❌ Erreur ajout action pendante: $e');
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getPendingActions() async {
    try {
      ...
      _log('❌ Erreur récupération actions pendantes: $e');
      return [];
    }
  }
  ...
  Future<void> _syncPendingActions() async {
    try {
      ...
      if (actions.isEmpty) {
        _log('ℹ️ Aucune action en attente');
        return;
      }

      _log('🔄 Synchronisation de ${actions.length} action(s)...');
      ...
      if (apiToken == null || baseUrl == null) {
        _log('⚠️ Impossible de synchroniser: pas de token API');
        return;
      }
      ...
      _log(
          '✅ Synchronisation terminée: $successCount réussies, $failureCount échouées');
    } catch (e) {
      _log('❌ Erreur synchronisation: $e');
    }
  }
    ...
    try {
      ...
      switch (method.toUpperCase()) {
        ...
        case 'DELETE':
          ...
          break;
        default:
          _log('⚠️ Méthode HTTP non supportée: $method');
          return false;
      }

      if (response.statusCode >= 200 && response.statusCode < 300) {
        _log('✅ Action synchronisée: ${action['type']}');
        return true;
      } else {
        _log('❌ Échec synchronisation: ${response.statusCode}');
        return false;
      }
    } catch (e) {
      _log('❌ Erreur exécution action pendante: $e');
      return false;
    }
  }
  ...
  Future<void> forceSyncNow() async {
    if (!_isOnline) {
      _log('⚠️ Impossible de synchroniser: pas de connexion');
      return;
    }

    _log('🔄 Synchronisation manuelle démarrée...');
  ...
  Future<dynamic> cacheFirst({
    ...
    if (_isOnline) {
      fetchFunction().then((data) {
        ...
        _log('⚠️ Erreur mise à jour cache: $e');
      });
    }
