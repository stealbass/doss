...
void main() {
  ...
  group('ApiHelpers - Formatting Tests', () {
    ...
    test('getRelativeTime should return correct relative time', () {
      ...
      // Days ago
      final daysAgo = now.subtract(Duration(days: 2));
      expect(ApiHelpers.getRelativeTime(daysAgo), 'Il y a 2 jours');

      // Weeks ago
      final weeksAgo = now.subtract(Duration(days: 14));
      expect(ApiHelpers.getRelativeTime(weeksAgo), 'Il y a 2 semaines');

      // Months ago (> 30 days)
      final monthsAgo = now.subtract(Duration(days: 45));
      expect(ApiHelpers.getRelativeTime(monthsAgo), contains('2025') || contains('2024'));
    });
  ...
  group('ApiHelpers - Timeout & Retry Tests', () {
    test('withTimeout should handle timeout correctly', () async {
      ...
      final fastOperation = Future.delayed(Duration(milliseconds: 100), () => 'success');
    ...
    test('withTimeout should throw on timeout', () async {
      ...
      final slowOperation = Future.delayed(Duration(seconds: 10), () => 'too slow');
    ...
    test('retryWithBackoff should throw after max attempts', () async {
      final alwaysFailOperation = () async {
