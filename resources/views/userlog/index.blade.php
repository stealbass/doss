            ...
            <div class="card shadow-none rounded-0 border-bottom">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center justify-content-end">
                        ...
                        <div class="col-xl-2 col-lg-3 col-md-6 col-sm-12 col-12 mx-2">
                            {{ Form::select('user', $usersList, isset($_GET['users']) ? $_GET['users'] : '', ['class' => 'form-control select multi-select', 'id' => 'id']) }}
                        </div>
                        <div class="row">
                            <div class="col-auto">
                                <div class="action-btn me-2">
                                    <button type="submit" class="mx-3 btn btn-sm btn-primary align-items-center "
                                        onclick="document.getElementById('userlogs_filter').submit(); return false;"
                                        data-bs-toggle="tooltip" title="{{ __('Apply') }}"
                                        data-title="{{ __('Apply') }}"><i class="ti ti-search"></i>
                                    </button>
                                </div>
                                <div class="action-btn me-2">
                                    <a href="{{ url('userlog') }}" data-bs-toggle="tooltip"
                                        title="{{ __('Reset') }}"data-title="{{ __('Reset') }}"
                                        class="mx-3 btn btn-sm btn-danger align-items-center "><i
                            ...
                            <tbody>
                                @foreach ($users as $user)
                                    <tr>
                                        ...
                                        <td><span
                                                class="me-5 badge p-2 px-3 bg-primary">{{ $userType->type }}</span>
                                        </td>
                                        <td>{{ $user->ip }}</td>
                                        <td>{{ $user->date }}</td>
                                        <td>{{ $json->country ?? '' }}</td>
                                        ...
                                        <td>
                                            <div class="action-btn me-2">
                                                <a href="#" data-size="md"
                                                    ...
                                                    data-ajax-popup="true" data-title="{{ __('View User Logs') }}"
