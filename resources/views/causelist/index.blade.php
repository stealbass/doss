    ...
    @can('create cause')
        <div class="text-sm-end d-flex all-button-box justify-content-sm-end">
            <a href="#" class="btn btn-sm btn-primary mx-1" data-ajax-popup="true" data-size="md"
                data-title="{{ __('Create Cause') }}" data-url="{{ route('cause.create') }}"
                                    ...
                                    <tr>
                                        <td>
                                            <a href="#" class="btn btn-sm"
                                                data-url="{{ route('cause.show', $caus->id) }}" data-size="md"
                                                data-ajax-popup="true" data-title="{{ __('View CauseList') }}">
                                            ...
                                            @can('delete cause')
                                                <div class="action-btn me-2">
                                                    <a href="#"
                                                        ...
                                                        data-confirm-yes="delete-form-{{ $caus->id }}"
                                                ...
                                                {!! Form::open([
                                                    'method' => 'DELETE',
                                                    'route' => ['cause.destroy', $caus->id],
                                                    'id' => 'delete-form-' . $caus->id,
                                                ]) !!}
                                                {!! Form::close() !!}
                                            @endcan
                                        </td>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        ...
                        $('#bench_div').empty();
                        $('#bench_div').append(
                            '<label for="bench" class="form-label">Circuit/Devision</label> <select class="form-control multi-select" name="bench" id="bench"> </select>'
                        );
        ...
        $(document).on('change', '#causelist_by', function() {
            ...
            switch (selectedValue) {
                case 'Advocate Name':
                    placeholderText = 'Enter Advocate Name';
                    break;
                case 'Keyword':
                ...
                case 'Party Name':
                    ...
                    break;
                case 'Judge Name':
                    placeholderText = 'Enter Judge Name';
                    break;
                default:
                    placeholderText = 'Enter Value';
            }

            $('#advocate_name').attr('placeholder', placeholderText);
        });
    </script>
@endpush
