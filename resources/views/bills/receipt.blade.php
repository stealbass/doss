    ...
    <div class="row justify-content-between align-items-center">
        <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
            <div class="action-btn mx-1">
                <a data-bs-toggle="tooltip" onclick="saveReceiptAsPDF()" class="btn btn-sm btn-primary" data-bs-placement="top"
            ...
            <div class="action-btn mx-1">
                <a href="{{ route('bills.show', $bill->id) }}" class="btn btn-sm btn-secondary" title="{{ __('Retour à la facture') }}"
...
@section('breadcrumb')
    <li class="breadcrumb-item">{{ __('Payment Receipt') }}</li>
@endsection
...
@section('content')
    <div class="row" id="receiptPrintableArea">
                ...
                <div class="receipt">
                    <div class="row receipt-header mt-2">
                        ...
                        <div class="col-12 text-center">
                            <h2 class="text-primary">{{ __('PAYMENT RECEIPT') }}</h2>
                        </div>
                        <div class="col-12">
                            <hr>
                        </div>
                    ...
                    <div class="row mt-3">
                        <div class="col-6">
                            <strong>{{ __('Receipt #') }}</strong> {{ sprintf('%05d', $payment->id) }}<br>
                            <strong>{{ __('Date') }}:</strong> {{ date('d/m/Y', strtotime($payment->date)) }}
                        </div>
                        <div class="col-6 text-end">
                            <strong>{{ __('Facture #') }}</strong> {{ $bill->bill_number }}<br>
                            <strong>{{ __('Méthode de paiement') }} :</strong> {{ $payment->method }}
                        </div>
                    </div>

                    <div class="row mt-4">
                        <div class="col-6">
                            <strong>{{ __('Informations de l’entreprise') }}</strong><br>
                            <small>
                                ...
                                <table class="table table-bordered">
                                    <thead class="bg-light">
                                        <tr>
                                            ...
                                            <th class="text-dark text-right" width="30%">{{ __('Montant') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>
                                                <strong>{{ __('Paiement pour la facture') }} {{ $bill->bill_number }}</strong><br>
                                    ...
                                    <tfoot class="bg-light">
                                        <tr>
                                            <th class="text-dark">{{ __('Montant total payé') }}</th>
                    ...
                    <div class="row mt-5">
                        <div class="col-6">
                            <div style="border-top: 1px solid #333; padding-top: 10px; width: 200px;">
                                <small>{{ __('Signature autorisée') }}</small>
                            </div>
                        </div>
                        <div class="col-6 text-end">
                    ...
                    <div class="row mt-4">
                        <div class="col-12 text-center">
                            <small class="text-muted">{{ __('Merci pour votre paiement') }}</small><br>
...
@push('custom-script')
    <style>
        /* PDF mode styles */
        #receiptPrintableArea.pdf-mode .col-md-2 {
            display: none !important;
        }
        
        #receiptPrintableArea.pdf-mode .col-md-8 {
            flex: 0 0 100% !important;
            max-width: 100% !important;
        }
        
        #receiptPrintableArea.pdf-mode .card {
            margin: 0 !important;
            padding: 20px !important;
        }
        
        #receiptPrintableArea.pdf-mode table {
            font-size: 11px !important;
            width: 100% !important;
        }
        
        #receiptPrintableArea.pdf-mode th,
        #receiptPrintableArea.pdf-mode td {
            padding: 10px !important;
        }
    </style>
    <script src="{{ asset('public/assets/js/html2pdf.bundle.js') }}"></script>
    <script>
        var filename = 'RECEIPT-{{ sprintf("%05d", $payment->id) }}-{{ $bill->bill_number }}';

        function saveReceiptAsPDF() {
            var element = document.getElementById('receiptPrintableArea');
