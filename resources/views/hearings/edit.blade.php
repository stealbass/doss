...
<div class="modal-body">
    <div class="row">
        ...
        <div class="form-group col-md-12">
            {{ Form::label('assigned_to', __('Assign To (Advocates/Members)'), ['class' => 'form-label']) }}
            @php
                $assignedIds = [];
                if (!empty($hearing->assigned_to)) {
                    if (is_string($hearing->assigned_to) && str_contains($hearing->assigned_to, ',')) {
                        $assignedIds = array_map('intval', explode(',', $hearing->assigned_to));
                    } else {
                        $assignedIds = [(int)$hearing->assigned_to];
                    }
                }
            @endphp
            <select class="form-control multi-select" name="assigned_to[]" id="assigned_to" placeholder="Select User" multiple>
                @if(isset($members))
        ...
        <div class="form-group col-md-12">
            ...
            {{ Form::textarea('remarks', null, ['class' => 'form-control summernote', 'rows' => '3']) }}
        </div>
