                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <ul class="mb-0 mt-2">
                                        ...
                                        <li>{{ __('Maximum file size: 200MB per file') }}</li>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="country" class="form-label"><strong>{{ __('Country') }} <span class="text-danger">*</span></strong></label>
                                    <select name="country" id="country" class="form-control" required>
                                        <option value="">{{ __('Select a country') }}</option>
                                        <option value="BJ" {{ $category->country == 'BJ' ? 'selected' : '' }}>🇧🇯 BJ - Bénin</option>
                                        <option value="BF" {{ $category->country == 'BF' ? 'selected' : '' }}>🇧🇫 BF - Burkina Faso</option>
                                        <option value="CM" {{ $category->country == 'CM' ? 'selected' : '' }}>🇨🇲 CM - Cameroun</option>
                                        <option value="CI" {{ $category->country == 'CI' ? 'selected' : '' }}>🇨🇮 CI - Côte d'Ivoire</option>
                                        <option value="CD" {{ $category->country == 'CD' ? 'selected' : '' }}>🇨🇩 CD - RD Congo</option>
                                        <option value="GA" {{ $category->country == 'GA' ? 'selected' : '' }}>🇬🇦 GA - Gabon</option>
                                        <option value="GW" {{ $category->country == 'GW' ? 'selected' : '' }}>🇬🇼 GW - Guinée-Bissau</option>
                                        <option value="MG" {{ $category->country == 'MG' ? 'selected' : '' }}>🇲🇬 MG - Madagascar</option>
                                        <option value="ML" {{ $category->country == 'ML' ? 'selected' : '' }}>🇲🇱 ML - Mali</option>
                                        <option value="MA" {{ $category->country == 'MA' ? 'selected' : '' }}>🇲🇦 MA - Maroc</option>
                                        <option value="NE" {{ $category->country == 'NE' ? 'selected' : '' }}>🇳🇪 NE - Niger</option>
                                        <option value="SN" {{ $category->country == 'SN' ? 'selected' : '' }}>🇸🇳 SN - Sénégal</option>
                                        <option value="TG" {{ $category->country == 'TG' ? 'selected' : '' }}>🇹🇬 TG - Togo</option>
                                        <option value="TN" {{ $category->country == 'TN' ? 'selected' : '' }}>🇹🇳 TN - Tunisie</option>
                                    </select>
                                    <small class="text-muted">{{ __('All uploaded documents will be tagged with this country') }}</small>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function handleFiles(files) {
        ...
        // Check file sizes (200MB max)
        const oversizedFiles = pdfFiles.filter(file => file.size > 200 * 1024 * 1024);
        if (oversizedFiles.length > 0) {
            alert('{{ __("Some files exceed the maximum size of 200MB:") }}\n' + 
