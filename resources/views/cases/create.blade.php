...
@section('page-title', __('Create Case'))

@section('breadcrumb')
    <li class="breadcrumb-item">{{ __('Create Case') }}</li>
@endsection
...
@section('content')
    {{ Form::open(['route' => 'cases.store', 'method' => 'post', 'enctype' => 'multipart/form-data', 'id' => 'documentForm', 'class' => 'needs-validation', 'novalidate']) }}
    <div class="row">
            ...
            <div class="card shadow-none rounded-0 border">
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                {{ Form::text('case_number', null, ['class' => 'form-control', 'placeholder' => __('Enter Case Number'), 'id' => 'case_number']) }}
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                {!! Form::label('year', __('Année de creation du dossier'), ['class' => 'form-label']) !!}<x-required></x-required>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12">
                            <div class="form-group">
                                {{ Form::label('advocates', __('Assigner à un juriste / employé'), ['class' => 'col-form-label']) }}
                                <select class="form-control multi-select" name="advocates[]" id="advocates" multiple
                                    placeholder="{{ __('Sélectionner juristes ou employés') }}">
                                    @foreach($assignables as $id => $name)
                                        <option value="{{ $id }}">{{ $name }}</option>
                                    @endforeach
                                </select>
                                <div class="text-xs mt-1">
                                    Crée un Juriste ou un Employé. <a class="dash-link" href="{{ route('advocate.index') }}"><b>Clique ici</b></a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4 choose-files fw-semibold ">
                            <label for="case_docs" class="upload__btn">
                                ...
                                <p>
                                    <span
                                        class="text-muted m-0 file-info">{{ __('Allowed file extension : ') }}{{ $file_validation['mimes'] }}</span><br>
                                    <span
            ...
            $("#documentForm").submit(function(event) {
                ...
                if (files) {
                    for (var i = 0; i < files.length; i++) {
                        var file = files[i];
                        // Cast to int server-side to avoid string * int TypeError
                        var maxSize = {{ (int) $file_validation['max_size'] }} * 1024;
