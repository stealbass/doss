                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12">
                            <div class="form-group">
                                ...
                                <select class="form-control multi-select" name="advocates[]" id="advocates" multiple
                                    ...
                                    @php
                                        $selectedIds = is_array($your_advocates) ? [] : $your_advocates->pluck('id')->toArray();
                                    @endphp
                                    @foreach($assignables as $id => $name)
                                        <option value="{{ $id }}" 
                                            {{ in_array($id, $selectedIds) ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
            ...
            $("#documentForm").submit(function(event) {
                ...
                if (files) {
                    for (var i = 0; i < files.length; i++) {
                        ...
                        // Cast to int server-side to avoid string * int TypeError and convert KB to bytes
