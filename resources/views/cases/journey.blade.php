                            ...
                            <div class="row btn-chk-work" role="group">
                                @foreach ($case->caseJourney() as $key => $step)
                                    <div class="col-md-2 col-sm-4 col-6">
                                        <input type="checkbox" class="btn-check" id="{{ $key }}" autocomplete="off"
                                            @if (in_array($key, explode(',', $case->journey))) checked @endif>
                                        <label
                                            class="btn btn-outline-primary p-3 w-100 d-flex justify-content-center align-items-center rounded-0 box"
                                            for="{{ $key }}" style="min-height: 100px; font-size: initial;">
                                            {{ $step }}
                                        </label>
                                    </div>
                                @endforeach
