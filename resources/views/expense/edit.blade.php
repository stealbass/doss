{{ Form::model($expense, ['route' => ['expenses.update', $expense->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'class' => 'needs-validation', 'novalidate']) }}
<div class="modal-body">
    <div class="row">
        <div class="form-group col-md-12">
            {!! Form::label('type', __('Type'), ['class' => 'form-label']) !!}
            <select class="form-control" name="type" id="type">
                <option value="expense" {{ $expense->type == 'expense' ? 'selected' : '' }}>{{ __('Dépense') }}</option>
                <option value="income" {{ $expense->type == 'income' ? 'selected' : '' }}>{{ __('Encaissement') }}</option>
            </select>
        </div>
        <div class="form-group col-md-12">
