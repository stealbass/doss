...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initCalculator() {
        ...
        if (!clientsInput || !planSelect || !monthlyEl || !yearlyEl || !passiveEl) {
        ...
        function calculateEarnings() {
            ...
            // Utiliser Intl.NumberFormat pour le formatage FCFA
            const formatter = new Intl.NumberFormat('fr-FR', {
                style: 'decimal',
                maximumFractionDigits: 0
            });

            monthlyEl.textContent = `${formatter.format(commission)} FCFA`;
            yearlyEl.textContent = `${formatter.format(yearly)} FCFA`;
            passiveEl.textContent = `${formatter.format(yearly)} FCFA`;
        }

        // Utiliser input au lieu de change pour mise à jour en temps réel
        clientsInput.addEventListener('input', calculateEarnings);
        planSelect.addEventListener('change', calculateEarnings);
