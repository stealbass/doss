...
class User extends Authenticatable implements MustVerifyEmail
{
     ...
     * Récupère la subscription la plus récente avec status='active'
     * et qui n'a pas expiré (expires_at NULL ou > now())
     */
    public function activeMobileSubscription()
    {
        return $this->hasOne(MobileAppSubscription::class)
                    ...
                    ->where(function($query) {
                        // Subscriptions sans date d'expiration (gratuit/pro) OU expiration future
                        $query->whereNull('expires_at')
                              ->orWhere('expires_at', '>', now());
                    })
                    ->latest('created_at'); // Prendre la plus récente
    }

    /**
     * Alias: mobileAppSubscription() pour compatibilité avec ancien code
     */
    public function mobileAppSubscription()
    {
        return $this->activeMobileSubscription();
    }

    /**
     * Get active mobile app subscription (instance, not relation)
     */
    public function getActiveMobileAppSubscription()
    {
        return $this->activeMobileSubscription()->first();
    }
    ...
    /**
     * Relation: Parrainages (alias pour referralsMade)
     */
    ...
    /**
     * Accessor: Retourne le nom du plan basé sur la subscription active
     * Utilisé par les contrôleurs API pour accéder à $user->plan
     * PRIORITE: subscription active > champ plan table users
     */
    public function getPlanAttribute()
    {
        // TOUJOURS vérifier d'abord la subscription active pour les utilisateurs mobiles
        if ($this->relationLoaded('activeMobileSubscription')) {
            $subscription = $this->getRelation('activeMobileSubscription');
            if ($subscription && $subscription->plan) {
                // Utiliser name_fr si disponible, sinon name
                return $subscription->plan->name_fr ?? $subscription->plan->name;
            }
        } else {
        ...
        // Fallback: champ plan dans la table users (legacy, pour les utilisateurs web)
        if (isset($this->attributes['plan']) && $this->attributes['plan']) {
    ...
    /**
     * Accessor: Retourne le pays (jurisdiction) de l'utilisateur
     * Utilisé par les contrôleurs API pour accéder à $user->country
     */
    public function getCountryAttribute()
    {
        // Si country existe déjà, on le retourne
        if (isset($this->attributes['country']) && $this->attributes['country']) {
            return $this->attributes['country'];
        }

        // Sinon, utiliser jurisdiction comme fallback
        return $this->attributes['jurisdiction'] ?? 'CM'; // CM par défaut (Cameroun)
    }
}
