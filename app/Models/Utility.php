...
class Utility extends Model
{
    ...
    {
        if (!Auth::check()) {
            return ''; // Return default or fallback value for guest
        }
    ...
    {
        $chatgpt_key = self::getValByName('chatgpt_key');
        $chatgpt_model = self::getValByName('chatgpt_model');

        if (\Auth::user()->type == 'super admin' || \Auth::user()->type == 'superAdminEmployee') {
            if (!empty($chatgpt_key) && !empty($chatgpt_model)) {
                return true;
            }
        } else {
            ...
            if ($enable_chatgpt == 'on' && !empty($chatgpt_key) && !empty($chatgpt_model)) {
                return true;
            }
        }
