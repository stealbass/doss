...
class DocumentTemplate extends Model
{
    ...
    {
        // Prefer configured storage (R2/S3/Wasabi); fallback to local public storage
    ...
    protected static function boot()
    {
        ...
        // Auto-index when template is created
        ...
        // Auto-reindex when template is updated with new extracted_text
        static::updated(function ($template) {
            if ($template->wasChanged('extracted_text') && !empty($template->extracted_text)) {
                (new \App\Services\AdvancedRagService())->deleteTemplate($template->id);
                ProcessTemplateForRAG::dispatch($template->id);
            }
        });

        // Clean up vectors on hard delete
        static::forceDeleting(function ($template) {
            (new \App\Services\AdvancedRagService())->deleteTemplate($template->id);
        });
    }
}
