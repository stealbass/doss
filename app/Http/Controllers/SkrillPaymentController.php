...
class SkrillPaymentController extends Controller
{
    ...
    public function invoicePayWithSkrill(Request $request)
    {
        ...
        $tran_id = md5(date('Y-m-d') . strtotime(datetime: 'Y-m-d H:i:s') . 'user_id');
    ...
    public function planPayWithSkrill(Request $request)
    {
        ...
        if ($plan) {
            ...
            $encryptedPlanId = urlencode($request->plan_id);
            ...
            // create object instance of SkrillRequest
            $skill->transaction_id = $tran_id; // generate transaction id
            $skill->amount = $price;
            $skill->currency = $this->currancy;
            $skill->language = 'EN';
            $skill->prepare_only = '1';
        ...
        if ($plan) {
            try {
                if (session()->has('skrill_data')) {
                    ...
                    $order->txn_id = $request->tansaction_id ?? '';
                    ...
                    $assignPlan = $user->assignPlan($plan->id, $request->payment_frequency ?? null);
                    ...
                    return redirect()->route('plans.index')->with('error', __('Session expired or payment not found.'));
                }
            } catch (Exception $e) {
