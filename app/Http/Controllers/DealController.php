...
class DealController extends Controller
{
    ...
    public function deal_list()
    {
        ...
        if ($usr->type == 'client') {
            $deals = Deal::select('deals.*')->join('client_deals', 'client_deals.deal_id', '=', 'deals.id')
                ->where('client_deals.client_id', '=', $usr->id)
                ->orderBy('deals.order')->get();
        } elseif (($usr->type == 'company' || $usr->type == 'superAdminEmployee') && !empty($pipeline) ) {
            $deals = Deal::where('created_by', '=', $usr->crmcreatorId())
                ->where('pipeline_id', '=', $pipeline->id)
                ->get();
        } else {
            $deals = Deal::select('deals.*')->join('user_deals', 'user_deals.deal_id', '=', 'deals.id')
                ->where('user_deals.user_id', '=', $usr->id)
                ->orderBy('deals.order')
                ->get();
        }

        if (!empty($pipeline)) {
            $total_deals = Deal::where('created_by', '=', $usr->crmcreatorId())->where('pipeline_id', '=', $pipeline->id)->get();
            ...
            // Deal Summary
            $cnt_deal = [];
            $cnt_deal['total'] = Deal::getDealSummary($total_deals);
            $cnt_deal['this_month'] = Deal::getDealSummary($curr_month);
            $cnt_deal['this_week'] = Deal::getDealSummary($curr_week);
            $cnt_deal['last_30days'] = Deal::getDealSummary($last_30days);
    ...
    public function update(Request $request, Deal $deal)
    {
        if (\Auth::user()->super_admin_employee == 1 ||
            ...
            $deal->phone_no = $request->phone_no;
            $deal->sources = !empty($request->sources) ? implode(",", array_filter($request->sources)) : '';
