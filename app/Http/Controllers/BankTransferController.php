...
class BankTransferController extends Controller
{
    ...
    public function planPayWithbank(Request $request)
    {
        if ($request->payment_receipt) {
            ...
            return redirect()->back()->with('error', __('Payment receipt required.'));
        }
        ...
        // Check for existing pending order and cancel it if found
        $order = Order::where('plan_id', $planID)->where('payment_status', 'Pending')->where('user_id', $authuser->id)->first();
        if ($order) {
            // Update the existing order to 'Cancelled' instead of blocking
        ...
        if ($request->payment_receipt) {
            ...
            if ($plan) {
                ...
                return redirect()->route('plans.index')->with('success', __('Plan payment request sent successfully!'));
            }
        }
    }
    ...
    public function invoicebankstatus(Request $request, $banktransfer_id)
    {
        ...
        if ($banktransfer) {
            ...
            if ($request->status == 'Approval') {
                ...
                $invoice_payment_ATTRIBUTES = date('Y-m-d');
