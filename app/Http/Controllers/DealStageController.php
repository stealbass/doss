...
class DealStageController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->super_admin_employee == 1) {
            ...
            return redirect()->route('dealStage.index')->with('success', __('Deal Stage successfully created.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function destroy(DealStage $dealStage)
    {
        if (\Auth::user()->super_admin_employee == 1) {
            ...
            return redirect()->route('dealStage.index')->with('success', __('Deal Stage successfully deleted.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
