...
class SendHearingReminderNotification implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            $hearingDate = \Carbon\Carbon::parse($this->hearing->date);
            $daysRemaining = now()->diffInDays($hearingDate, false);

            // Send email to each user
            foreach ($usersToNotify as $user) {
                if (!empty($user->email)) {
                    Mail::to($user->email)->send(new HearingReminderMail($this->hearing, $case, $user, $daysRemaining));
                    Log::info("Hearing reminder sent to: {$user->email} ({$daysRemaining} days before hearing)");
                }
            }

        } catch (\Exception $e) {
            Log::error("Error sending hearing reminder notification: " . $e->getMessage());
        }
    }
}
